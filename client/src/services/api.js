...
const mockRecipes = [
  {
    ...
    instructions: "Cook pasta according to package. Heat sauce. Combine and serve.",
  ...
  },
  {
    ...
    instructions: "Sauté vegetables. Add broth. Simmer until vegetables are tender.",
  ...
  },
  {
    ...
    instructions: "Butter bread. Add cheese between slices. Grill until golden and cheese is melted.",
...
export const fetchRecipes = async (query) => {
  try {
    ...
    const mealCategories = ["Breakfast", "Lunch", "Dinner", "Desserts", "Vegetarian", "Quick Meals"];
    if (mealCategories.includes(query)) {
      ...
      console.log(`Fetching recipes for category: ${query} (tag: ${tagMap[query]})`);
      ...
      const mockCategoryMap = {
        ...
        "Vegetarian": ["Vegetable Soup", "Pasta with Tomato Sauce", "Berry Smoothie"],
        "Quick Meals": ["Grilled Cheese Sandwich", "Berry Smoothie"]
      };
      
      const categoryMockRecipes = mockRecipes.filter(recipe => 
        mockCategoryMap[query] && mockCategoryMap[query].includes(recipe.name)
      );
      
      const data = await callSpoonacularAPI(url, { recipes: categoryMockRecipes });
      ...
      return (data.recipes || []).map((recipe) => ({
        ...
        image: recipe.image,
        ingredients: recipe.extendedIngredients?.map((ing) => ing.original) || recipe.ingredients,
        instructions: recipe.instructions || "Instructions not available",
        estimatedTime: recipe.readyInMinutes ? `${recipe.readyInMinutes} mins` : recipe.estimatedTime,
        servings: recipe.servings,
      }));
    }

    // Process query - could be a single term or multiple ingredients
    let searchTerms = query.split(',').map(term => term.trim()).filter(term => term.length > 0);
  ...
  try {
    ...
    const data = await callSpoonacularAPI(
      ...
      mockRecipes.filter(r => {
        ...
        if (Array.isArray(ingredients)) {
          ...
          return r.ingredients.some(i => i.toLowerCase().includes(ingredientsParam.toLowerCase()));
        }
      })
    );
    ...
    const detailedRecipes = await Promise.all(
      data.map(async (recipe) => {
        ...
        const mockDetailedRecipe = mockRecipes.find(r => r._id === recipe.id?.toString()) || {
        ...
        const detailData = await callSpoonacularAPI(detailUrl, mockDetailedRecipe);

        return {
          _id: recipe.id?.toString() || recipe._id,
          name: detailData.title || recipe.title || recipe.name,
          image: recipe.image,
          ingredients: detailData.extendedIngredients?.map(ing => ing.original) || 
                      recipe.usedIngredients?.concat(recipe.missedIngredients).map(ing => ing.original) || 
                      recipe.ingredients || [],
          instructions: detailData.instructions || "Instructions not available",
          estimatedTime: detailData.readyInMinutes ? `${detailData.readyInMinutes} mins` : "N/A",
...
export const getRecipeById = async (recipeId) => {
  try {
    ...
    const mockRecipe = mockRecipes.find(r => r._id === recipeId) || mockRecipes[0];
...
export const generateRecipe = async (ingredients, preferences = null) => {
  try {
    ...
    let url = `${SPOONACULAR_BASE_URL}/recipes/findByIngredients?apiKey=${SPOONACULAR_API_KEY}&ingredients=${encodeURIComponent(ingredientsParam)}&number=1&ranking=1&ignorePantry=false`;
    
    // Add dietary preferences if provided
    if (preferences) {
      url = `${SPOONACULAR_BASE_URL}/recipes/complexSearch?apiKey=${SPOONACULAR_API_KEY}&includeIngredients=${encodeURIComponent(ingredientsParam)}&diet=${encodeURIComponent(preferences)}&number=1&addRecipeInformation=true&fillIngredients=true`;
    }
    ...
    const mockFilter = () => {
      ...
      const ingredientsArray = typeof ingredientsParam === 'string' 
      ...
      const scoredRecipes = mockRecipes.map(recipe => {
        ...
        ingredientsArray.forEach(ingredient => {
          if (recipe.ingredients.some(i => i.toLowerCase().includes(ingredient.toLowerCase()))) {
            score++;
          }
        });
        return { recipe, score };
    ...
    if (preferences) {
      ...
      if (!data.results || data.results.length === 0) {
        throw new Error("No recipes found with those ingredients and preferences");
      }
    ...
    return {
      ...
      ingredients: recipeDetail.extendedIngredients?.map(ing => ing.original) || recipeDetail.ingredients,
      instructions: recipeDetail.instructions || "Instructions not available",
      summary: recipeDetail.summary,
      estimatedTime: recipeDetail.readyInMinutes ? `${recipeDetail.readyInMinutes} mins` : recipeDetail.estimatedTime,
      servings: recipeDetail.servings,
      estimatedCalories: Math.round(recipeDetail.nutrition?.nutrients?.find(n => n.name === "Calories")?.amount) || 500,
    ...
    return {
      ...
      diets: ["mockDiet"],
      summary: `A delicious recipe featuring ${Array.isArray(ingredients) ? ingredients.join(", ") : ingredients}.`
    };
  }
};
