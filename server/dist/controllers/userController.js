...
export const updateUserProfile = async (req, res, next) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(req.user?.id, { name, email }, { new: true, runValidators: true }).select("-password_hash");
        if (!updatedUser) {
            res.status(404).json({ success: false, message: "User not found" });
            return;
        }
...
export const saveRecipe = async (req, res, next) => {
    try {
        ...
        if (user.savedRecipes.some((saved) => saved.toString() === recipe._id.toString())) {
 ...
 */
export const removeSavedRecipe = async (req, res, next) => {
    try {
        const { recipeId } = req.params;
        if (!mongoose.Types.ObjectId.isValid(recipeId)) {
            res.status(400).json({ success: false, message: "Invalid recipe ID" });
 ...
 */
export const addCalorieLog = async (req, res, next) => {
    try {
 ...
 * Get calorie log
 */
export const getCalorieLog = async (req, res, next) => {
    try {
